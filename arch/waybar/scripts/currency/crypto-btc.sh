...
function getRoundedCurrency {
	btcvalue=`cat ${currencyJsonPath} | jq -r '.data[0]|"\(.values.USD.price)"'`
    btcarrow=`cat ${currencyJsonPath} | jq -r '.data[0]|"\(.values.USD.percentChange24h)"'`

	roundValue=`echo "scale=2;($btcvalue)/1" | bc`
    ...
    result="$roundValue`getCurrencyArrow $btcarrow`"
    echo $result
}
...
function saveCurrency {
	...
	echo `getRoundedCurrency` > "$contextPath/crypto-btc"
}
